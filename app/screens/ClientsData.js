...
export const fetchClientData = async (spreadsheetId) => {
    ...
    try {
        ...
        return {
            ...
            SumOfSales: formatCurrency(values[1]), // ✅ USD formatted
            SumOfProfits: formatCurrency(values[2]), // ✅ USD formatted
            AveragePrice: formatCurrency(values[3]), // ✅ USD formatted
            AverageProfit: formatCurrency(values[4]), // ✅ USD formatted
            TotalHoursWorked: values[5] || "0",
            AmountOfProducts: values[6] || "0",
            DatePurchased: convertGoogleSheetsDate(values[7]), // ✅ Convert to date
            HoursPurchased: values[8] || "0",
            HoursLeftInBundle: values[9] || "0",
            SalesPast30Days: formatCurrency(values[10]), // ✅ USD formatted
            ProfitsPast30Days: formatCurrency(values[11]), // ✅ USD formatted
            ManagementCost30Days: formatCurrency(values[12]), // ✅ USD formatted
        };
    } catch (error) {
